...
rotateOptions.forEach(option => {
    option.addEventListener("click", () => {
        if(option.id === "left") {
            rotate -= 90;
        } else if(option.id === "right") { // if clicked btn is right rotate,increment rotate value by +90
            rotate += 90;
        } else if(option.id === "horizontal") {
            flipHorizontal = flipHorizontal === 1 ? -1 : 1; // if flipHorizontal value is 1,set this value to -1 else set1
        } else {
...
const resetFilter = () =>{
    ...
    filterOptions[0].click(); //  clicking brightness button so that the brightness selected by default 
    applyFilter();
}
